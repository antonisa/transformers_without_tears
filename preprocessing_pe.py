...
if __name__ == '__main__':
    ...
    for pair in pairs:
        ...
        lang = lans[1]
        # We probably don't wont to oversample from the second language and to use the MT output when we learn the BPE
        #infile = join(data_dir, '{}/train.guess.{}'.format(pair, lang))
        #with open(infile, 'r') as fin:
        #    datas[lang].extend(fin.readlines())
        infile = join(data_dir, '{}/train.target.{}'.format(pair, lang))
        with open(infile, 'r') as fin:
            datas[lang].extend(fin.readlines())
    ...
    for pair in pairs:
        ...
        bpe_vocab_file = bpe_vocab_files[lang]
        for mode in [ac.TRAIN, ac.DEV, ac.TEST]:
            infile = join(data_dir, '{}/{}.guess.{}'.format(pair, mode, lang))
            encoded_file = '{}.bpe'.format(infile)
            command = '{} applybpe {} {} {} {}'.format(fast, encoded_file, infile, code_file, bpe_vocab_file)
            print(command)
            subprocess.check_call(command, shell=True)
        bpe_vocab_file = bpe_vocab_files[lang]
        for mode in [ac.TRAIN, ac.DEV, ac.TEST]:
            infile = join(data_dir, '{}/{}.target.{}'.format(pair, mode, lang))
            encoded_file = '{}.bpe'.format(infile)
            command = '{} applybpe {} {} {} {}'.format(fast, encoded_file, infile, code_file, bpe_vocab_file)
            print(command)
            subprocess.check_call(command, shell=True)
    ...
    for pair in pairs:
        ...
        lang = langs[1]
        infile = join(data_dir, '{}/train.guess.{}.bpe'.format(pair, lang))
        with open(infile, 'r') as fin:
            for line in fin:
                toks = line.strip().split()
                if toks:
                    joint_vocab.update(toks)
                    sub_vocabs[lang].update(toks)
        infile = join(data_dir, '{}/train.target.{}.bpe'.format(pair, lang))
        with open(infile, 'r') as fin:
            for line in fin:
                toks = line.strip().split()
                if toks:
    ...
    for pair in pairs:
        for mode in [ac.TRAIN, ac.DEV]:
            ...
            guess_infile = join(data_dir, '{}/{}.guess.{}.bpe'.format(pair, mode, tgt_lang))
            tgt_infile = join(data_dir, '{}/{}.target.{}.bpe'.format(pair, mode, tgt_lang))
            with open(src_infile, 'r') as f_src, open(guess_infile, 'r') as f_guess, open(tgt_infile, 'r') as f_tgt:
                for src_line, guess_line, tgt_line in zip(f_src, f_guess, f_tgt):
                    src_toks = src_line.strip().split()
                    guess_toks = guess_line.strip().split()
                    tgt_toks = tgt_line.strip().split()

                    if src_toks and tgt_toks and guess_toks:
                        src_toks = [joint_vocab.get(tok, ac.UNK_ID) for tok in src_toks] + [ac.EOS_ID]
                        guess_toks = [joint_vocab.get(tok, ac.UNK_ID) for tok in guess_toks] + [ac.EOS_ID]
                        tgt_toks = [ac.BOS_ID] + [joint_vocab.get(tok, ac.UNK_ID) for tok in tgt_toks]
                        src_data.append(src_toks)
                        guess_data.append(guess_toks)
                        tgt_data.append(tgt_toks)

            src_data = np.array(src_data)
            guess_data = np.array(guess_data)
            tgt_data = np.array(tgt_data)
            src_npy_file = join(data_dir, '{}/{}.{}.npy'.format(pair, mode, src_lang))
            guess_npy_file = join(data_dir, '{}/{}.guess.{}.npy'.format(pair, mode, src_lang))
            tgt_npy_file = join(data_dir, '{}/{}.target.{}.npy'.format(pair, mode, tgt_lang))
            np.save(src_npy_file, src_data)
            np.save(guess_npy_file, guess_data)
