...
from layers import Encoder, Decoder, PEDecoder
import utils as ut
...
class PETransformer(nn.Module):
    ...
    def forward(self, src, guess, tgt, targets, src_lang_idx, tgt_lang_idx, logit_mask):
        embed_dim = self.args.embed_dim
        max_len = max(src.size(1), guess.size(1), tgt.size(1))
        pos_embedding = ut.get_positional_encoding(embed_dim, max_len)
        word_embedding = F.normalize(self.word_embedding, dim=-1) if self.args.fix_norm else self.word_embedding
        ...
        decoder_mask = torch.triu(torch.ones((tgt.size(-1), tgt.size(-1))), diagonal=1).type(tgt.type()) == 1
        decoder_mask = decoder_mask.unsqueeze(0).unsqueeze(1)
        decoder_outputs = self.decoder(decoder_inputs, decoder_mask, encoder1_outputs, encoder1_mask, encoder2_outputs, encoder2_mask)
